...
function upperkeysarray($testa)
{
    ...
    for ($x=0;$x<sizeof($newkeys);$x++){
        $newtesta[mb_strtoupper($newkeys[$x])]=$testa[$newkeys[$x]];
    }
    return $newtesta;
}
...
function insertmysentence($mybigsentence)
{
    ...
    for ($x=0;$x<sizeof($allwords)+1;$x++){
        ...
        if (mb_strtoupper($word)=="<INPUT>"){
            $sentencepart="INPUT";
        } elseif (mb_strtoupper($word)=="<THAT>"){
            $sentencepart="THAT";
        } elseif (mb_strtoupper($word)=="<TOPIC>"){
            $sentencepart="TOPIC";
        }
        ...
        elseif ((mb_strtoupper($allwords[$x+1])=="<THAT>") || (mb_strtoupper($allwords[$x+1])=="<TOPIC>")){
            $lwic=1;
        }
...
function startS($parser,$name,$attrs)
{
    ...
    if (mb_strtoupper($name)=='LEARN') {
        $whaton = 'LEARN';
    }
    if (mb_strtoupper($name)=="GENDER"){
        $startupwhich="GENDER";
    }
    elseif (mb_strtoupper($name)=="INPUT"){
        $startupwhich="INPUT";
    }
    elseif (mb_strtoupper($name)=="PERSON"){
        $startupwhich="PERSON";
    }
    elseif (mb_strtoupper($name)=="PERSON2"){
        $startupwhich="PERSON2";
    }

    if (mb_strtoupper($name)=="PROPERTY"){
    ...
    elseif (mb_strtoupper($name)=="SPLITTER"){
        $splitterarray[]=$attrs["VALUE"];
    }
    elseif (mb_strtoupper($name)=="SUBSTITUTE"){
        if (trim($attrs["FIND"])!=""){
...
function startElement($parser, $name, $attrs) 
{
    ...
    if (mb_strtoupper($name)=="CATEGORY"){
        $whaton="CATEGORY";
    }
    elseif (mb_strtoupper($name)=="TEMPLATE"){
        $whaton="TEMPLATE";
        $template="";
        $recursive=0;
    }
    elseif (mb_strtoupper($name)=="PATTERN"){
        $whaton="PATTERN";
    }
    elseif ((mb_strtoupper($name)=="THAT")&&(mb_strtoupper($whaton)!="TEMPLATE")){
        $whaton="THAT";
    }
    elseif (mb_strtoupper($name)=="TOPIC"){
        $whaton="TOPIC";
    }

    if ((mb_strtoupper($whaton)=="PATTERN")&&(mb_strtoupper($name)!="PATTERN")){


        if (mb_strtoupper($name)=="BOT"){
    ...
    elseif ((mb_strtoupper($whaton)=="TEMPLATE")&&(mb_strtoupper($name)!="TEMPLATE")){
    ...
    elseif (mb_strtoupper($whaton)=="TOPIC"){
...
function handleme($parser, $data)
{
    ...
    if (mb_strtoupper($whaton)=="PATTERN"){
        $pattern .= $data;
    }
    elseif (mb_strtoupper($whaton)=="TOPIC"){
        $topic .= $data;
    }
    elseif (mb_strtoupper($whaton)=="THAT"){
        $that .= $data;
    }
    elseif (mb_strtoupper($whaton)=="TEMPLATE"){
        $template .= $data;
    }
	elseif (mb_strtoupper($whaton)=="TEMPLATE"){
		$template .= "<![CDATA[$data]]>";
...
function learn($file)
{
    ...
    if (mb_strtoupper(substr($file,0,7))=="HTTP://"){
        $file=$file;
    }
    else {
        $file=$rootdir . $file;
