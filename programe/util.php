...
function iscustomtag($tagname,&$functocall){
	...
	if (in_array(mb_strtoupper($tagname),$cttags)){
...
function loadcustomtags(){
	...
	foreach($definedfuncs as $x){
		if (substr($x,0,3)=="ct_"){
			$cttags[]=mb_strtoupper(substr($x,3,strlen($x)));
		}
	}
...
/**
 * M.P.
 * Parse to replace some key strings 
 */
        ...
        foreach($tokens as $token){
            if ($token[0] === '{' && $token[strlen($token)-1] === '}') {
                ...
                switch($t[0]){
                    case 'image':
                        $file = $t[1];
                        $input .= '<br/><a class="image" target="_blank" href="'.$ourl.$file.'"><img style="width:100%" src="'.$ourl.$file.'" /></a><br/>';
                        break;
                    case 'video':
                        $file = $t[1];
                        $poster = preg_replace('/\..+$/', '.' . 'jpg', $file);
                        $input .= '<br /><video style="width:100%" controls="controls" poster="'.$ourl.$poster.'">
                            <source src="'.$ourl.$file.'" type="video/mp4" />
                        </video><br />
                        ';
                        break;
                }
            } else {
                if(trim($token)!="" && strlen($token)>1)
                    $input .= $token;
            }
        }
    }
